...
def on_mqtt_connect(
    ...
    if "on_connect" in auto_messages:
        for item in auto_messages["on_connect"]:
            client.publish(
                prefix + item["topic"], item["payload"], item["qos"], item["retain"]
            )
...
if "last_will" in auto_messages:
    mqttc.will_set(
        prefix + auto_messages["last_will"]["topic"],
        auto_messages["last_will"]["payload"],
        auto_messages["last_will"]["qos"],
        auto_messages["last_will"]["retain"],
    )
